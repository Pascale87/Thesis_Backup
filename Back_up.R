...
Newborn_with_HHH_Check_id <- Newborn_with_HHH %>% 
  select(patient_id_child, case_id_child) %>% 
  distinct # 136
...
Newborn_with_HHH_rf_Check_id <- Newborn_with_HHH_rf %>% 
  select(patient_id_child, case_id_child) %>% 
  distinct # 137
...
rm(Newborn_fully_healthy_Check_id, Newborn_with_HHH_Check_id, Newborn_no_HHH_with_rf_Check_id, Newborn_with_HHH_rf_Check_id)
...
## Merge together the different newborn groups and prepared dataset (Sample3) --> to be discussed
...
# To exclude the missing temperature data entry cases
Sample3 <- Sample3 %>%
  filter(!is.na(Hypothermia_cat)) # 5718
...
# 121             216           3751           1780            50 

# 3.5.3 Hyperbilirubinaemia categories ----------------------------------------

summary(as.factor(Sample3$Bilirubin_cat))
...
Sample4 <- Sample3 %>%
  ...
  mutate(hypoglyc_y = Hypoglycaemia_cat %in% c("Mild", "Moderate", "Severe"),
         hyperbili_y = if_else(is.na(Bilirubin_cat), FALSE, Bilirubin_cat %in% c("Hyperbilirubinaemia_serum", "Hyperbilirubinaemia_tcb")),
...
## ICD code related to HHH summarised
...
# To identify newborns only with diagnosis "Einling, Geburt im Krankenhaus"
...
Sample_test <- left_join(Sample2, Newborn_red, by = c("patient_id_child", "case_id_child")) # 6836
...
summary(as.factor(Sample_test3$Hypoglycaemia_cat))
# Mild       Moderate    No_measurement    Normoglycaemic     Severe 
# 172             25           4311           2233            95 

# 3.4.4.2 Hyperbilirubinaemia categories ----------------------------------------

summary(as.factor(Sample_test4$Hyperbili_cat))
# Hyperbili         No_measurement        Physiological_jaundice 
# 55                687                   6094 
## Conclusion: random check showed that there are cases with no documentation of bilirubin levels - so it was summarised that these would indicate also no measurement during hospital stay
...
summary(as.factor(Sample_test5$Hypothermia_cat))
# Mild    Moderate_Severe   Norm            NA's 
# 982     532               5310            12   

Missing_temp <- Sample_test5 %>% 
  filter(is.na(Hypothermia_cat))
## 5 cases yes Nicu admission (2 of them stayed for one hour in the postnatal unit), 6 of them ambulant/tagesklinik, 1 child no data entry
...
# Merge with Sample
Sample_test6 <- left_join(Sample_test6, Parity, by = c("patient_id_child", "case_id_child" = "case_id", "patient_id_mother", "case_id_mother")) %>% 
  select(-Anzahl_vorausg_SS, -Anzahl_vorausg_LebGeb, -Anzahl_fehl_Geb, -Anzahl_Interruptio)
...
Sample_test6 <- left_join(Sample_test6, Mother_data, by = c("patient_id_mother", "case_id_mother")) %>% # without date of birth!
  select(- Birth_date, - PAT_BIRTH_DATE, - CBIS_BIRTH_DATE_TS.y) %>% 
  rename(CBIS_BIRTH_DATE_TS = CBIS_BIRTH_DATE_TS.x)
...
# Combine with Sample
Sample_test6 <- left_join(Sample_test6, Countries2b, by = "PAT_CITIZENSHIP_COUNTRY") %>% 
  select(- PAT_CITIZENSHIP_COUNTRY, - pat_citizenship_country_new, -landercode_bfs_code_des_pays_ofs_codice_del_paese_ust, 
         -iso2, -iso3, -region_region_regione, - kontinent_continent_continente) 

table(Sample_test6$Country)
# Africa     America      Asia      Europe    Oceania   Switzerland     Unknown 
# 262         186         503       2936      16        2916            17 
round(prop.table(table(as.factor(Sample_test6$Country))) * 100, 1)
# Africa     America      Asia      Europe     Oceania    Switzerland     Unknown 
...
## To exclude cases with maternal risk factors for hypoglycaemia (and hyperbilirubinaemia)
Sample_maternal_rf <- left_join(Sample_test6, Diagnose_red2_corr, by = c("patient_id_mother" = "patient_id", "case_id_mother" = "case_id"), relationship = "many-to-many") %>%
  select(- DIG_DATE_TS, - DIG_RANK, - DIG_TARGET_SITE, - DIA_BK, - ICD_groups) %>% 
  rename(DIA_NK_maternal= DIA_NK, ICD_labels_maternal = ICD_labels) %>%
  relocate(DIA_NK_maternal, ICD_labels_maternal, .after = admission_neo_n) %>%
...
## Next step: merging Hypoglyc_data_cat with Sample/newborn groups (attention: without variable birth date! It doesn't work with these variables, 
...
# Groups
Newborn_group1 <- left_join(Newborn_with_HHH, Hypoglyc_data_cat,  by = c("patient_id_child" = "patient_id", "case_id_child" = "case_id")) %>%
  mutate(Hypoglycaemia_cat = if_else(is.na(Hypoglycaemia_cat), "No_measurement", Hypoglycaemia_cat)) %>% 
  select(- pat_geb_dat, - alter_in_tagen)
Newborn_group2 <- left_join(Newborn_with_HHH_rf, Hypoglyc_data_cat,  by = c("patient_id_child" = "patient_id", "case_id_child" = "case_id")) %>%
  mutate(Hypoglycaemia_cat = if_else(is.na(Hypoglycaemia_cat), "No_measurement", Hypoglycaemia_cat)) %>% 
  select(- pat_geb_dat, - alter_in_tagen)
Newborn_group3 <- left_join(Newborn_no_HHH_with_rf, Hypoglyc_data_cat,  by = c("patient_id_child" = "patient_id", "case_id_child" = "case_id")) %>%
  mutate(Hypoglycaemia_cat = if_else(is.na(Hypoglycaemia_cat), "No_measurement", Hypoglycaemia_cat)) %>% 
  select(- pat_geb_dat, - alter_in_tagen)
Newborn_group4 <- left_join(Newborn_fully_healthy, Hypoglyc_data_cat,  by = c("patient_id_child" = "patient_id", "case_id_child" = "case_id")) %>%
  mutate(Hypoglycaemia_cat = if_else(is.na(Hypoglycaemia_cat), "No_measurement", Hypoglycaemia_cat)) %>% 
  select(- pat_geb_dat, - alter_in_tagen)
...
#   filter(admission_neo %in% "Yes", Hypoglycaemia_cat %in% c("Normoglycaemic", "Mild", "Moderate", "Severe")) # 60 cases, without "No_measure"
# 
# table(Hypoglyc_neo$Hypoglycaemia_cat)
# # Mild  Normoglycaemic         Severe 
# # 1             36             23  
# 
...
#   filter(admission_neo %in% "Yes", Hypoglycaemia_cat %in% c("Normoglycaemic", "Mild", "Moderate", "Severe", "No_measure")) # 99 cases, with "No_measure"
# table(Hypoglyc_neo2$Hypoglycaemia_cat)
# # Mild     No_measure Normoglycaemic         Severe 
# # 1             39             36             23 
# 
...
rm(Bloodmeasure_n, Bloodmeasure_y, Hypoglyc_cat, Lookup_hypoglyc, Lookup_hypoglyc2)
...
Photo_USB <- Bili_procedure %>% 
  ...
  mutate(Phototherapy_ind_usb = case_when(Hyperbilirubinaemia_cat %in% c("Hyperbilirubinaemia_serum", "Hyperbilirubinaemia_tcb") ~ TRUE, TRUE ~ FALSE),
         ...
         Phototherapy_status = case_when(Phototherapy_ind_usb & Phototherapy_doc ~ "Indicated_AND_Doc",
                                         Phototherapy_ind_usb & !Phototherapy_doc ~ "Indicated_BUT_Not_Doc",
                                         !Phototherapy_ind_usb & Phototherapy_doc ~ "Not_Indicated_BUT_Doc",
...
# 1 Indicated_AND_Doc                        22
# 2 Indicated_BUT_Not_Doc                    45
# 3 Not_Indicated_BUT_Doc                    62
...
Newborn_bili_status <- left_join(Bili_cat_USB_measure3, Photo_USB, by = c("patient_id", "case_id", "Bilirubin_cat"))
...
# Groups
Newborn_group1 <- left_join(Newborn_group1, Newborn_bili_status, by = c("patient_id_child" = "patient_id", "case_id_child" = "case_id")) %>%
  mutate(Hyperbilirubinaemia_cat = if_else(is.na(Hyperbilirubinaemia_cat), "No_measurement", Hyperbilirubinaemia_cat),
         Phototherapy_status = if_else(is.na(Phototherapy_status), "No_measurement", Phototherapy_status))
Newborn_group2 <- left_join(Newborn_group2, Newborn_bili_status,  by = c("patient_id_child" = "patient_id", "case_id_child" = "case_id")) %>%
  mutate(Hyperbilirubinaemia_cat = if_else(is.na(Hyperbilirubinaemia_cat), "No_measurement", Hyperbilirubinaemia_cat),
         Phototherapy_status = if_else(is.na(Phototherapy_status), "No_measurement", Phototherapy_status))
Newborn_group3 <- left_join(Newborn_group3, Newborn_bili_status,  by = c("patient_id_child" = "patient_id", "case_id_child" = "case_id")) %>%
  mutate(Hyperbilirubinaemia_cat = if_else(is.na(Hyperbilirubinaemia_cat), "No_measurement", Hyperbilirubinaemia_cat),
         Phototherapy_status = if_else(is.na(Phototherapy_status), "No_measurement", Phototherapy_status))
Newborn_group4 <- left_join(Newborn_fully_healthy, Newborn_bili_status,  by = c("patient_id_child" = "patient_id", "case_id_child" = "case_id")) %>%
  mutate(Hyperbilirubinaemia_cat = if_else(is.na(Hyperbilirubinaemia_cat), "No_measurement", Hyperbilirubinaemia_cat),
         Phototherapy_status = if_else(is.na(Phototherapy_status), "No_measurement", Phototherapy_status))
...
# Left_join back to my sample to add variable (bring together with Sample2b, Replace NA to "no_measurement")
Sample_test4 <- left_join(Sample_test3, Hyperbilirubinaemia_cat, by = c("patient_id_child" = "patient_id", "case_id_child" = "case_id")) %>% 
  mutate(Hyperbili_cat = replace_na(Hyperbili_cat, "No_measurement"))
...
## n=1 Nicu admission, n=6 ambulant/tagesklinik, 1 child no data entry
# 
table(Sample3$Hypothermia_cat)
# Mild Moderate_Severe          Norm 
# 816             446            4456   
# 
round(prop.table(table(as.factor(Sample3$Hypothermia_cat))) * 100, 1)
# # Mild     Moderate_Severe    NSample3# # Mild     Moderate_Severe    Norm 
# # 14.3           7.8           77.9
# 
Hypotherm_Nicu <- Sample3 %>%
  filter(admission_neo %in% "Yes", Hypothermia_cat %in% c("Norm", "Mild", "Moderate_Severe")) # 47
# 
